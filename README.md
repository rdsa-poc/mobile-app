...
- Start the app with matching `--dart-define` flags so the Flutter shell receives the same keys used by the other repos. Prefer the bundled SDK to avoid relying on a global Flutter installation:
  `../flutter/bin/flutter run --dart-define=RADIOSA_ENVIRONMENT=local --dart-define=RADIOSA_APP_ID=app --dart-define=RADIOSA_REALTIME_BASE_URL=http://localhost:5001`
